...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  ...
  {
    index = (c - 'a' + 37) * 8; // Para letras minúsculas, assumindo que a tabela contém caracteres minúsculos
  }

  // Desenha o caractere
  for (uint8_t i = 0; i < 8; ++i)
  {
    uint8_t line = font[index + i]; // Recupera a linha da fonte
    for (uint8_t j = 0; j < 8; ++j)
    {
      ssd1306_pixel(ssd, x + i, y + j, line & (1 << j)); // Desenha o pixel correspondente
    }
  }
}
